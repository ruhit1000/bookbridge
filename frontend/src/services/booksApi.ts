...
export const booksApi = {
    getBooks: async (status?: BookStatus) => {
    ...
    createBook: async (data: { title: string; author: string; description?: string; condition?: string; categoryId: string }) => {
        const response = await api.post<ApiResponse<Book>>("/books", data);
        return response.data;
    },
    updateBook: async (id: string, data: Partial<{ title: string; author: string; description: string; condition: string; categoryId: string; status: BookStatus }>) => {
